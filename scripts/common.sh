...
KEYFILE=$TMP_PATH/DecoratesPortal.Presentation.key
CRTFILE=$TMP_PATH/DecoratesPortal.Presentation.crt
PFXFILE=$TMP_PATH/DecoratesPortal.Presentation.pfx
...
configure_nssdb() {
    ...
    certutil -d sql:"$1" -D -n DecoratesPortal.Presentation
    certutil -d sql:"$1" -A -t "CP,," -n DecoratesPortal.Presentation -i $CRTFILE
}
...
if [ "$SAVE" = 1 ]; then
   ...
   cp $PFXFILE $HOME/.aspnet/https
   echo "Saved certificate to $HOME/.aspnet/https/$(basename $PFXFILE)"
fi
